...
const PAIRING_CODE_TTL_MS=24*60*60*1000,ADMIN_SESSION_TTL_MS=30*24*60*60*1000;
const PAIRING_ALPHABET="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
const pairingCodes=new Map<string,{adminToken:string;expiresAt:number}>();
let failedPairWindowAt=0,failedPairAttempts=0;
...
export async function handleAdminRequest(request:Request,url:URL,adminToken:string|undefined,info:AdminServerInfo,world:AdminWorldControl):Promise<Response|null>{
  ...
  if(request.method==="POST"&&url.pathname==="/admin/api/pair"){
    const now=Date.now();if(now-failedPairWindowAt>60_000){failedPairWindowAt=now;failedPairAttempts=0}
    if(failedPairAttempts>=30)return jsonError("pairing_rate_limited",429);
    const body=await smallJson(request),code=text(body?.code,16).toUpperCase(),entry=pairingCodes.get(code);
    if(!entry||entry.adminToken!==adminToken||entry.expiresAt<=now){failedPairAttempts+=1;return jsonError("invalid_pairing_code",401)}
    pairingCodes.delete(code);return Response.json({ok:true,sessionToken:await issueAdminSession(adminToken),expiresAt:now+ADMIN_SESSION_TTL_MS},{headers:SECURITY_HEADERS});
  }
  const rootAuthorized=authorizedRoot(request,adminToken);
  if(request.method==="POST"&&url.pathname==="/admin/api/pair-code"){
    if(!rootAuthorized)return Response.json({ok:false,error:"unauthorized"},{status:401,headers:{...SECURITY_HEADERS,"www-authenticate":"Bearer"}});
    const now=Date.now();for(const [code,entry] of pairingCodes)if(entry.expiresAt<=now||entry.adminToken===adminToken)pairingCodes.delete(code);
    const code=createPairingCode();pairingCodes.set(code,{adminToken,expiresAt:now+PAIRING_CODE_TTL_MS});
    return Response.json({ok:true,code,expiresAt:now+PAIRING_CODE_TTL_MS},{headers:SECURITY_HEADERS});
  }
  if(!rootAuthorized&&!await authorizedSession(request,adminToken))return Response.json({ok:false,error:"unauthorized"},{status:401,headers:{...SECURITY_HEADERS,"www-authenticate":"Bearer"}});
...
function secureEqual(left:string,right:string){const n=Math.max(left.length,right.length);let d=left.length^right.length;for(let i=0;i<n;i++)d|=(left.charCodeAt(i)||0)^(right.charCodeAt(i)||0);return d===0}
function authorizedRoot(request:Request,expected:string){return secureEqual(bearer(request),expected)}
function createPairingCode(){const bytes=crypto.getRandomValues(new Uint8Array(10));return [...bytes].map(value=>PAIRING_ALPHABET[value%PAIRING_ALPHABET.length]).join("")}
...
async function sessionSignature(adminToken:string,payload:string){const key=await crypto.subtle.importKey("raw",new TextEncoder().encode(adminToken),{name:"HMAC",hash:"SHA-256"},false,["sign"]);return randomHexFrom(await crypto.subtle.sign("HMAC",key,new TextEncoder().encode(payload)))}
function randomHexFrom(value:ArrayBuffer){return [...new Uint8Array(value)].map(byte=>byte.toString(16).padStart(2,"0")).join("")}
async function issueAdminSession(adminToken:string){const expiresAt=Date.now()+ADMIN_SESSION_TTL_MS,nonce=randomHex(16),payload=`${expiresAt}.${nonce}`;return `v1.${payload}.${await sessionSignature(adminToken,payload)}`}
async function authorizedSession(request:Request,adminToken:string){const [version,expiresSource,nonce,signature,...rest]=bearer(request).split(".");if(rest.length||version!=="v1"||!/^\d{13}$/.test(expiresSource??"")||!/^[a-f0-9]{32}$/.test(nonce??"")||!/^[a-f0-9]{64}$/.test(signature??""))return false;const expiresAt=Number(expiresSource),now=Date.now();if(expiresAt<=now||expiresAt>now+ADMIN_SESSION_TTL_MS+60_000)return false;return secureEqual(signature,await sessionSignature(adminToken,`${expiresSource}.${nonce}`))}
...
function setting(a,b){const n=el('div',undefined,'setting');n.append(el('span',a,'sub'),el('b',b));return n}function fmt(n){return Number(n).toFixed(1)}async function refresh(){if(!secret)return locked();try{const d=await api('/admin/api/state');signal.className='signal online';q('#signalText').textContent='ONLINE';render(d)}catch(e){signal.className='signal bad';q('#signalText').textContent=e.message==='unauthorized'?'PAIR DEVICE':'OFFLINE'}}function locked(){signal.className='signal';q('#signalText').textContent='PAIR DEVICE';players.replaceChildren(el('div','Console locked.','empty'))}q('#unlock').onclick=pair;token.onkeydown=e=>{if(e.key==='Enter')pair()};q('#forget').onclick=()=>{secret='';token.value='';localStorage.removeItem('lakecraft_admin_session');localStorage.removeItem('lakecraft_admin_token');locked()};q('#run').onclick=()=>command(q('#command').value.trim());q('#command').onkeydown=e=>{if(e.key==='Enter')command(e.currentTarget.value.trim())};document.querySelectorAll('[data-cmd]').forEach(b=>b.onclick=()=>command(b.dataset.cmd));refresh();setInterval(()=>{if(secret)refresh()},2500);
</script></body></html>`;
