...
The realtime server persists bounded shared item drops in the world SQLite database; nearby players can exchange exact stacks through server-validated drop and pickup operations. Selected items, canonical swing/use actions, and explicit crouch state are relayed so remote avatars use the shared third-person rig. Multiplayer mobs currently use the browser's single-player simulation, and PvP is visual-only while server combat authority is still being built—neither path calls obsolete Lakebed presence/combat mutations. Authored block edits, trees, and cave interiors do not yet alter server collision. Reconnect credentials expire after ten minutes, and block/drop interactions are reach-limited against the accepted player pose.
