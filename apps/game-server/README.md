...
Set `ADMIN_TOKEN` to a separate secret of at least 24 characters to enable the per-world command deck at `https://YOUR-SERVER/admin`. It persists spawn/time/daylight, public/password/whitelist/closed access, operators and moderators; exposes server chat; changes game modes; and can kick, ban, or pardon players. The token is sent only in an Authorization header and defaults to session-only browser storage; **Remember device** is explicit.
