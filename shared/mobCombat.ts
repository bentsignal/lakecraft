...
function isMobKind(value: string): value is MobAuthorityKind {
  return value === "pig" || value === "cow" || value === "sheep" || value === "chicken" || value === "zombie"
...
export function validateMobIdentity(
  ...
  const match = /^(pig|cow|sheep|chicken|zombie|skeleton|creeper|spider)-([0-9a-z]{1,8})-([0-9a-z]{1,3})$/.exec(mobId);
