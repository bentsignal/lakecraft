...
async function authoritativeRangedOccluders(
  ...
  if (probeOverflow || cells.size === 0) return { ok: false, reason: BS.invalidWorldProbe };
  ...
  if (groups.size > RANGED_MAX_PROBE_CHUNKS) return { ok: false, reason: BS.invalidWorldProbe };
  for (const [chunkKey, group] of groups) {
    const rows = await newestByIndex(db.worldChunks, "by_chunk", (q) => q.eq("chunkKey", chunkKey))
      .take(2);
    if (rows.length > 1) return { ok: false, reason: BS.duplicateWorldState };
    if (rows.length === 0) {
...
async function applyAuthoritativeWorldExplosion(
  ...
  if (groups.size > 4) return { ok: false, reason: BS.invalidWorldProbe };
  for (const [chunkKey, group] of groups) {
  ...
  for (const cell of plannedDestruction) {
    const rows = await newestByIndex(db.worldEdits, BS.byCoord, (q) => q.eq(BS.coordKey, cell.coordKey)).take(2);
    if (rows.length > 1) return { ok: false, reason: BS.duplicateWorldState };
    const existing = rows[0] ?? null;
  ...
  for (const candidate of potentialVictims) {
    ...
    const combatRows = await newestByIndex(db.playerCombat, BS.byUser, (q) => q.eq(BS.userId, candidate.row.userId)).take(2);
    const inventoryRows = await newestByIndex(db.inventories, BS.byUser, (q) => q.eq(BS.userId, candidate.row.userId)).take(2);
    ...
    if (!inventoryState.ok) return { ok: false, reason: BS.targetStateInvalid };
...
async function authoritativeFallWorldFacts(
  ...
  for (const [chunkKey, group] of groups) {
    const chunkRows = await newestByIndex(db.worldChunks, "by_chunk", (q) => q.eq("chunkKey", chunkKey))
      .take(2);
    chunkReads += 1;
    if (chunkRows.length > 1) return { ok: false, reason: BS.duplicateWorldState };
...
async function authorizeInventoryCraftingTable(
  ...
  const presenceRows = await newestByIndex(db.playerPresence, BS.byUser, (q) => q.eq(BS.userId, userId))
    .take(2);
...
export default capsule({
  ...
  schema: {
     ...
     * The preserved incident-compatible tables now power the reviewed signed-in backup protocol.
     */
  ...
  queries: {
    ...
    worldChunks: query(async (ctx, rawChunkKeys: string[]) => {
      ...
      if (fuses.length > 0) {
        const presenceRows = await newestByIndex(ctx.db.playerPresence, BS.byHeartbeat, (q) => q.gte(BS.heartbeatAt, String(serverNow - ACTIVE_PLAYER_WINDOW_MS)))
          .take(64);
    ...
    multiplayerComposite: query(async (ctx, requestJson: string) => {
      ...
      const activeRows = await newestByIndex(ctx.db.playerPresence, BS.byHeartbeat, (q) => q.gte(BS.heartbeatAt, String(serverNow - ACTIVE_PLAYER_WINDOW_MS)))
        .take(128);
    ...
    singlePlayerCloudBackups: query(async (ctx) => {
      ...
      if (!inventory.ok || inventory.stateBytes > SINGLE_PLAYER_CLOUD_BACKUP_MAX_USER_STATE_BYTES) {
        ...
        const revision = quota.length === 1 && /^\d{1,16}$/.test(quota[0].revision)
          && Number(quota[0].revision) <= Number.MAX_SAFE_INTEGER ? quota[0].revision : "0";
        return [3, serverNow, revision] as const;
      }
      const backups = [];
      const descriptors = [];
      for (const world of inventory.worlds) {
        const loaded = loadSinglePlayerCloudBackupParts(ctx.auth.userId, world.parts);
        if (!loaded.ok) {
          descriptors.push([3, world.worldId, "0"]);
          continue;
        }
        if (loaded.accountFence) {
          descriptors.push([2, loaded.accountFence.revision]);
          continue;
        }
        if (loaded.tombstones.length) {
          const tombstone = loaded.tombstones[0];
          descriptors.push([1, tombstone.worldId, tombstone.revision,
            tombstone.deletedRevision, tombstone.deletedAt]);
          continue;
        }
        const [{ manifest, snapshotJson }] = loaded.backups;
        if (!manifest || Number(manifest.uploadedAt) > serverNow) {
          descriptors.push([3, world.worldId, manifest?.revision ?? "0"]);
          continue;
        }
        backups.push(singlePlayerCloudBackupWire(manifest, snapshotJson));
      }
      backups.sort((left, right) => left[1] < right[1] ? -1 : left[1] > right[1] ? 1 : 0);
      return [1, serverNow, backups, descriptors] as const;
    })
  },
  ...
  mutations: {
    ...
    mutateSinglePlayerCloudBackup: mutation(async (ctx, requestJson: string) => {
      const serverNow = Date.now();
      if (!ctx.auth.isAuthenticated || ctx.auth.isGuest) {
        return [4, serverNow] as const;
      }
      const userId = ctx.auth.userId;
      if (typeof userId !== "string" || userId.length < 1 || userId.length > 520) {
        return [5, BS.invalidServerState, serverNow] as const;
      }
      const disposition = parseSinglePlayerCloudDispositionRequest(requestJson);
      const deleting = disposition ? null : parseSinglePlayerCloudBackupDeleteRequest(requestJson);
      const parsed = disposition || deleting ? null : parseSinglePlayerCloudBackupCommitRequest(requestJson);
      if (!disposition && !deleting && !parsed?.ok) {
        return [5, parsed?.reason ?? BS.invalidRequest, serverNow] as const;
      }
      const worldId = deleting?.[1] ?? parsed?.candidate.worldId ?? "";
      const expectedRevision = deleting?.[2] ?? parsed?.candidate.expectedRevision ?? "0";
      ...
      const validBudget = (row: typeof userBudget) => {
        if (!row || typeof row.userId !== "string" || row.userId.length < 1 || row.userId.length > 520
          || !/^\d{1,2}$/.test(row.acceptedToday) || Number(row.acceptedToday) > 12
          || !/^\d{1,16}$/.test(row.lastAcceptedAt) || Number(row.lastAcceptedAt) > serverNow
          || (row.activeBackup !== "0" && row.activeBackup !== "1") || !/^\d{16}$/.test(row.cleanupAfter)) return !row;
        const cleanupDue = singlePlayerCloudBudgetCleanupAfter(row.dayKey, Number(row.lastAcceptedAt));
        return cleanupDue !== null && (row.activeBackup === "1" ? row.cleanupAfter === "9999999999999999"
          : Number.isSafeInteger(Number(row.cleanupAfter)) && Number(row.cleanupAfter) >= cleanupDue
            && Number(row.cleanupAfter) <= 8_640_000_000_000_000);
      };
      if (!validBudget(userBudget)) {
        return [5, BS.invalidServerState, serverNow] as const;
      }
      const validQuota = !quota || (quota.quotaKey === SINGLE_PLAYER_CLOUD_QUOTA_KEY
        && /^\d{1,6}$/.test(quota.activeStateBytes) && validUtcCloudBackupDay(quota.dayKey) && /^\d{1,3}$/.test(quota.acceptedToday)
        && Number(quota.acceptedToday) <= 120 && /^\d{1,16}$/.test(quota.lastAcceptedAt) && Number(quota.lastAcceptedAt) <= serverNow
        && utcCloudBackupDay(Number(quota.lastAcceptedAt)) === quota.dayKey
        && /^\d{1,16}$/.test(quota.revision)
        && validSinglePlayerCloudQuotaState(Number(quota.activeStateBytes), SINGLE_PLAYER_CLOUD_BACKUP_QUOTA_STATE_BYTES,
          Number(quota.revision)));
      if (!validQuota || (!quota && (userParts.length > 0 || userBudget || cleanupCandidates.length > 0))) {
        return [5, BS.invalidServerState, serverNow] as const;
      }
      const quotaRevision = Number(quota?.revision ?? "0");
      const proposedRevision = nextSinglePlayerCloudGeneration(quotaRevision);
      if (!proposedRevision) {
        return [5, BS.invalidServerState, serverNow] as const;
      }
      if (disposition) {
        if (!quota || disposition[1] !== quota.revision) return [5, "conflict", serverNow] as const;
        const loaded = loadSinglePlayerCloudBackupParts(userId, userParts);
        if (disposition[0] === 3) {
          if (!loaded.ok || !loaded.accountFence || loaded.accountFence.revision !== disposition[1]
            || loaded.backups.length || loaded.tombstones.length) return [5, "conflict", serverNow] as const;
          const fence = loaded.accountFence;
          const fenceRows = userParts.filter((row) => row.worldId === SINGLE_PLAYER_CLOUD_ACCOUNT_FENCE_WORLD);
          const floor = SINGLE_PLAYER_CLOUD_BACKUP_QUOTA_STATE_BYTES
            + (userBudget ? SINGLE_PLAYER_CLOUD_BACKUP_BUDGET_STATE_BYTES : 0);
          const nextActive = Math.max(floor, Number(quota.activeStateBytes)
            - fenceRows.reduce((sum, row) => sum + cloudBackupStoredPartBytes(userId, row.worldId, row.part, row.data), 0));
          ...
          await ctx.db.singlePlayerCloudBackupQuota.update(quota.id, { ...quota,
            activeStateBytes: String(nextActive), revision: proposedRevision });
          return [8, proposedRevision, serverNow] as const;
        }
        ...
        for (const row of batch) {
          removedCharge += row.data.length > SINGLE_PLAYER_CLOUD_BACKUP_MAX_GLOBAL_STATE_BYTES
            ? SINGLE_PLAYER_CLOUD_BACKUP_MAX_GLOBAL_STATE_BYTES
            : cloudBackupStoredPartBytes(userId, row.worldId, row.part, row.data);
        }
        const budgetCharge = SINGLE_PLAYER_CLOUD_BACKUP_BUDGET_STATE_BYTES;
        let addedCharge = userBudget ? 0 : budgetCharge;
        let fenceHeader = "";
        if (!continuation) {
          fenceHeader = singlePlayerCloudTombstoneHeader({ worldId: SINGLE_PLAYER_CLOUD_ACCOUNT_FENCE_WORLD,
            revision: proposedRevision, deletedRevision: disposition[1], deletedAt: String(serverNow),
            operationId: `recover_${proposedRevision}` });
        ...
        const nextActive = Math.max(floor, Number(quota.activeStateBytes) - removedCharge + addedCharge);
        if (!validSinglePlayerCloudQuotaState(nextActive, floor, Number(proposedRevision))) {
          return [5, BS.invalidServerState, serverNow] as const;
        }
        ...
        if (!continuation) await ctx.db.singlePlayerCloudBackupParts.insert({
          userId, worldId: SINGLE_PLAYER_CLOUD_ACCOUNT_FENCE_WORLD, part: "0", data: fenceHeader,
        });
        await ctx.db.singlePlayerCloudBackupQuota.update(quota.id, { ...quota,
          activeStateBytes: String(nextActive), revision: proposedRevision });
        if (!userBudget) await ctx.db.singlePlayerCloudBackupBudgets.insert({ userId, dayKey: "1970-01-01",
          acceptedToday: "0", lastAcceptedAt: "0", activeBackup: "1", cleanupAfter: "9999999999999999" });
      ...
      if (!inventory.ok) return [5, BS.invalidServerState, serverNow] as const;
      const reconstructed = inventory.worlds.map((world) => ({ world,
        loaded: loadSinglePlayerCloudBackupParts(userId, world.parts) }));
      const healthyBackups = reconstructed.flatMap(({ loaded }) => loaded.ok ? loaded.backups : []);
      const tombstones = reconstructed.flatMap(({ loaded }) => loaded.ok ? loaded.tombstones : []);
      const accountFence = reconstructed.flatMap(({ loaded }) => loaded.ok && loaded.accountFence
        ? [loaded.accountFence] : [])[0] ?? null;
      const targetWorld = inventory.worlds.find((world) => world.worldId === worldId) ?? null;
      const currentBackup = healthyBackups.find(({ manifest }) => manifest.worldId === worldId) ?? null;
      const currentTombstone = tombstones.find((tombstone) => tombstone.worldId === worldId) ?? null;
      const current = currentBackup?.manifest ?? null;
      const currentParts = targetWorld?.parts ?? [];
      const currentSnapshotJson = currentBackup?.snapshotJson ?? "";
      const activeWorldCount = healthyBackups.length;
      const allHealthy = reconstructed.every(({ loaded }) => loaded.ok);
      if (accountFence || (!deleting && (!allHealthy
        || healthyBackups.some(({ manifest }) => Number(manifest.uploadedAt) > serverNow)))
        || deleting && reconstructed.some(({ world, loaded }) => world.worldId !== worldId && !loaded.ok)) {
        return [5, BS.invalidServerState, serverNow] as const;
      }
      if (healthyBackups.some(({ manifest }) => Number(manifest.revision) > quotaRevision)
        || tombstones.some((tombstone) => Number(tombstone.revision) > quotaRevision)) {
        return [5, BS.invalidServerState, serverNow] as const;
      }
      const userWorldCount = inventory.worlds.length;
      let cleanupCharge = 0;
      const cleanupRows = [];
      const cleanupOwners = new Set<string>();
      for (const row of cleanupCandidates) {
        if (!validBudget(row) || cleanupOwners.has(row.userId)
          || row.userId === userId && (!deleting || current || currentTombstone)) continue;
        cleanupOwners.add(row.userId);
        const ownerBackup = await newestByIndex(ctx.db.singlePlayerCloudBackupParts,
          BS.byUser, (q) => q.eq(BS.userId, row.userId)).take(1);
        if (ownerBackup.length !== 0) continue;
        cleanupRows.push(row);
        cleanupCharge += SINGLE_PLAYER_CLOUD_BACKUP_BUDGET_STATE_BYTES;
      }
      const callerBudgetCleaned = Boolean(userBudget && cleanupRows.some((row) => row.id === userBudget.id));
      ...
      const userStateBytes = inventory.stateBytes;
      const minimumStateBytes = SINGLE_PLAYER_CLOUD_BACKUP_QUOTA_STATE_BYTES + userStateBytes
        + (userBudget && !callerBudgetCleaned ? SINGLE_PLAYER_CLOUD_BACKUP_BUDGET_STATE_BYTES : 0);
      const recoveringDelete = Boolean(deleting && targetWorld);
      if (!Number.isSafeInteger(globalStateBytes) || globalStateBytes < 0
        ...
        || (!recoveringDelete && userStateBytes > SINGLE_PLAYER_CLOUD_BACKUP_MAX_USER_STATE_BYTES)
        || (!recoveringDelete && quota
          && !validSinglePlayerCloudQuotaState(globalStateBytes, minimumStateBytes, Number(quota.revision)))
        || (userWorldCount > 0 && (!userBudget || userBudget.activeBackup !== "1"))) {
        return [5, BS.invalidServerState, serverNow] as const;
      }
      const applyCleanup = async () => {
        for (const row of cleanupRows) await ctx.db.singlePlayerCloudBackupBudgets.delete(row.id);
      };
      const nextQuotaValue = (activeStateBytes: number, minimumBytes: number, nextDayKey: string,
        acceptedToday: number, lastAcceptedAt: number) => {
        return validSinglePlayerCloudQuotaState(activeStateBytes, minimumBytes, Number(proposedRevision))
          && validUtcCloudBackupDay(nextDayKey) && Number.isSafeInteger(acceptedToday) && acceptedToday >= 0
          && acceptedToday <= 120 && Number.isSafeInteger(lastAcceptedAt) && lastAcceptedAt >= 0
          && lastAcceptedAt <= serverNow && utcCloudBackupDay(lastAcceptedAt) === nextDayKey
          ? { quotaKey: SINGLE_PLAYER_CLOUD_QUOTA_KEY, activeStateBytes: String(activeStateBytes), dayKey: nextDayKey,
            acceptedToday: String(acceptedToday), lastAcceptedAt: String(lastAcceptedAt), revision: proposedRevision }
          : null;
      };
      if (deleting) {
        if (currentTombstone) {
          return currentTombstone.deletedRevision === expectedRevision
            && currentTombstone.operationId === deleting[3]
            ? [2, currentTombstone.revision, serverNow] as const
            : [5, "conflict", serverNow] as const;
        }
        const deleteDecision = decideSinglePlayerCloudBackupDeleteRevision(Boolean(targetWorld), current?.revision ?? null,
          expectedRevision);
        if (deleteDecision === "conflict") return [5, "conflict", serverNow] as const;
        if (!targetWorld) {
          if (cleanupRows.length && quota) {
            const nextQuota = nextQuotaValue(globalStateBytes, SINGLE_PLAYER_CLOUD_BACKUP_QUOTA_STATE_BYTES,
              quota.dayKey, Number(quota.acceptedToday), Number(quota.lastAcceptedAt));
            if (!nextQuota) return [5, BS.invalidServerState, serverNow] as const;
            await applyCleanup();
            await ctx.db.singlePlayerCloudBackupQuota.update(quota.id, nextQuota);
          }
          return [2, "0", serverNow] as const;
        }
        if (tombstones.length >= SINGLE_PLAYER_CLOUD_BACKUP_MAX_WORLDS) {
          return [3, "cloud_capacity", serverNow] as const;
        }
        const tombstone = { worldId, revision: proposedRevision, deletedRevision: expectedRevision,
          deletedAt: String(serverNow), operationId: deleting[3] };
        const tombstoneHeader = singlePlayerCloudTombstoneHeader(tombstone);
        const tombstoneCharge = cloudBackupStoredPartBytes(userId, worldId, "0", tombstoneHeader);
        const nextUserStateBytes = userStateBytes - targetWorld.stateBytes + tombstoneCharge;
        const remainingMinimum = SINGLE_PLAYER_CLOUD_BACKUP_QUOTA_STATE_BYTES + nextUserStateBytes
          + SINGLE_PLAYER_CLOUD_BACKUP_BUDGET_STATE_BYTES;
        const nextActiveStateBytes = globalStateBytes - targetWorld.stateBytes + tombstoneCharge;
        const nextQuota = nextQuotaValue(nextActiveStateBytes, remainingMinimum,
          quota!.dayKey, Number(quota!.acceptedToday), Number(quota!.lastAcceptedAt));
        if (!nextQuota) return [5, BS.invalidServerState, serverNow] as const;
        for (const row of currentParts) await ctx.db.singlePlayerCloudBackupParts.delete(row.id);
        await ctx.db.singlePlayerCloudBackupParts.insert({ userId, worldId, part: "0", data: tombstoneHeader });
        await applyCleanup();
        await ctx.db.singlePlayerCloudBackupQuota.update(quota!.id, nextQuota);
        return [2, proposedRevision, serverNow] as const;
      }
      const parsedCandidate = parsed!.candidate;
      if (currentTombstone && parsedCandidate.expectedRevision !== currentTombstone.revision) {
        return [5, "conflict", serverNow] as const;
      }
      const proposedHeader = singlePlayerCloudBackupHeader({
        worldId, name: parsedCandidate.name, seed: String(parsedCandidate.seed), gameMode: parsedCandidate.gameMode,
        worldCreatedAt: String(parsedCandidate.worldCreatedAt), snapshotHash: parsedCandidate.snapshotHash,
        snapshotUtf8Bytes: String(parsedCandidate.snapshotUtf8Bytes), stateBytes: "0",
        chunkCount: String(parsedCandidate.chunks.length), revision: proposedRevision, uploadedAt: String(serverNow),
      });
      const candidate = { ...parsedCandidate, expectedRevision: currentTombstone ? "0" : parsedCandidate.expectedRevision,
        stateBytes: parsedCandidate.chunks.reduce((sum, chunk, index) => sum
          + cloudBackupStoredPartBytes(userId, worldId, String(index + 1), chunk),
        cloudBackupStoredPartBytes(userId, worldId, "0", proposedHeader)) };
      ...
      const replacedStateBytes = currentTombstone ? targetWorld?.stateBytes ?? 0 : 0;
      const decision = decideSinglePlayerCloudBackupCommit(current, current ? currentSnapshotJson : null, candidate,
        activeWorldCount, userStateBytes - replacedStateBytes, admissionStateBytes - replacedStateBytes, userLastAcceptedAt,
        userAcceptedToday, globalAcceptedToday, proposedRevision, serverNow);
      if (!decision.ok) return decision.reason === "cadence"
        ? [6, decision.retryAfterMs!, serverNow] as const
        : decision.reason === "cloud_capacity" || decision.reason === "world_limit"
          ? [3, decision.reason, serverNow] as const : [5, decision.reason, serverNow] as const;
      if (decision.kind === "deduped") {
        if (cleanupRows.length && quota) {
          const nextQuota = nextQuotaValue(globalStateBytes, minimumStateBytes, quota.dayKey,
            Number(quota.acceptedToday), Number(quota.lastAcceptedAt));
          if (!nextQuota) return [5, BS.invalidServerState, serverNow] as const;
          await applyCleanup();
          await ctx.db.singlePlayerCloudBackupQuota.update(quota.id, nextQuota);
        }
        return [1, decision.manifest.revision, serverNow] as const;
      }
      const oldStateBytes = current ? Number(current.stateBytes) : replacedStateBytes;
      const nextActiveStateBytes = admissionStateBytes - oldStateBytes + candidate.stateBytes;
      const nextUserStateBytes = userStateBytes - oldStateBytes + candidate.stateBytes;
      const nextQuota = nextQuotaValue(nextActiveStateBytes,
        SINGLE_PLAYER_CLOUD_BACKUP_QUOTA_STATE_BYTES + nextUserStateBytes + SINGLE_PLAYER_CLOUD_BACKUP_BUDGET_STATE_BYTES,
        dayKey, globalAcceptedToday + 1, serverNow);
      if (!nextQuota) return [5, BS.invalidServerState, serverNow] as const;
      for (const row of currentParts) await ctx.db.singlePlayerCloudBackupParts.delete(row.id);
      await ctx.db.singlePlayerCloudBackupParts.insert({
        userId, worldId, part: "0", data: singlePlayerCloudBackupHeader(decision.manifest),
      });
      for (let index = 0; index < candidate.chunks.length; index += 1) {
        await ctx.db.singlePlayerCloudBackupParts.insert({
          userId, worldId, part: String(index + 1), data: candidate.chunks[index],
        });
      }
      await applyCleanup();
    ...
    growOakTree: mutation(async (
      ...
      if (!pose) return { ok: false, reason: BS.invalidPose, serverNow };
    ...
    editWorldBlock: mutation(async (
      ...
      const pose = validatePresencePoseFields(poseX, poseY, poseZ, poseYaw, posePitch);
      if (!pose) return { ok: false, reason: BS.invalidPose };
      ...
      if (effect.kind === "place" && effect.nextBlock === "sapling") {
        if (request.y <= WORLD_EDIT_MIN_Y) return { ok: false, reason: BS.invalidSupport };
        ...
        if (supportBlock !== "grass" && supportBlock !== "dirt") {
          return { ok: false, reason: BS.invalidSupport };
        }
      }
      ...
      if (effect.kind === "mine" && effect.previousBlock === "furnace") {
        ...
        if (recoveryStacks.length > 0) {
          ...
          if (recoveryStacks.some((_, index) => !canCreateDroppedItem(activeOwnedDrops.length + index))) {
            return { ok: false, reason: BS.dropLimit };
          }
    ...
    startPresenceSession: mutation(async (ctx, rawSessionId: string) => {
      ...
      const rows = await newestByIndex(ctx.db.playerPresence, BS.byUser, (q) => q.eq(BS.userId, ctx.auth.userId))
        .take(64);
    ...
    authorizeRespawn: mutation(async (ctx, rawSessionId: string) => {
        ...
        if (structurallyValidGrant && activeGrant.consumedAt
          ...
          if (combatRow && combatRow.health !== "0" && target && activeGrant.consumedAt && currentPose
            ...
            if (!validPresenceSessionId(presence.sessionId) || nextPoseSequence === null) {
              return { ok: false, reason: BS.invalidPresenceState };
            }
            return {
              ok: true,
              target,
      ...
      if (!presenceIsActive) return { ok: false, reason: BS.activePresenceRequired };
      ...
      if (!deathPose) return { ok: false, reason: BS.invalidPresenceState };
      ...
      if (!deathPlan.ok) return { ok: false, reason: BS.invalidDeathSettlement };
      const activeOwnedDrops = await ctx.db.droppedItems
        .withIndex("by_owner_expiry", (q) => q
          .eq("ownerUserId", ctx.auth.userId)
          .gt("expiresAt", String(serverNow)))
      ...
      if (previousEpoch === null) return { ok: false, reason: BS.invalidState };
      const epoch = nextWorldBlockRevision(previousEpoch);
      if (epoch === null) return { ok: false, reason: BS.invalidState };
      const grant = buildPresenceRelocationGrant(ctx.auth.userId, epoch, destination, serverNow);
      if (!grant) return { ok: false, reason: BS.authorizationFailure };
      const respawnSessionId = `${presence.sessionId.slice(0, 36)}-${epoch}`;
      if (!validPresenceSessionId(respawnSessionId)) return { ok: false, reason: BS.authorizationFailure };
      const value = {
        userId: ctx.auth.userId,
    ...
    applyInventoryAction: mutation(async (ctx, requestJson: string) => {
      ...
      if (currentRevision === null) return { ok: false, reason: BS.invalidState };
      ...
      if (request.action.kind === "initialize") {
        ...
        if (!previous.ok) return { ok: false, reason: BS.invalidState };
    ...
    dropItem: mutation(async (ctx, requestJson: string) => {
      ...
      const activeOwnedDrops = await ctx.db.droppedItems
        ...
        .order("asc")
        .take(65);
      if (!canCreateDroppedItem(activeOwnedDrops.length)) return { ok: false, reason: BS.dropLimit };
    ...
    operateFurnace: mutation(async (ctx, requestJson: string) => {
      ...
      const furnace = blockInstanceToken
        ? materializedFurnaceView(furnaceRows[0] ?? null, coordinate.coordKey, blockInstanceToken, serverNow)
        : null;
      if (!furnace) return { ok: false, reason: BS.invalidState, serverNow };
    ...
    sleepInBed: mutation(async (ctx, rawCoordKey: string) => {
      ...
      if (ownPresences.length !== 1) return { ok: false, reason: BS.activePresenceRequired };
    ...
    checkpointMobWorld: mutation(async (ctx, requestJson: string) => {
      ...
      if (!stored) {
        if (existing || request.expectedRevision !== 0) {
          return { ok: false, reason: existing ? BS.invalidCheckpoint : BS.revisionConflict, serverNow };
        }
        ...
        if (!state) return { ok: false, reason: BS.initializationFailed, serverNow };
        const replayInput = await readCurrentReplayInput();
        if (!replayInput) return { ok: false, reason: BS.initializationFailed, serverNow };
      ...
      if (!nextReplayInput) return { ok: false, reason: BS.invalidReplayInput, serverNow };
    ...
    claimMobPlayerDamage: mutation(async (ctx, requestJson: string) => {
      ...
      if (!callerTarget) return { ok: false, reason: BS.activePresenceRequired, serverNow };
      ...
      const inventoryRow = inventoryRows[0];
      const playerState = validatePlayerStateJson(inventoryRow.inventoryJson);
      if (!playerState.ok) return { ok: false, reason: BS.targetStateInvalid, serverNow };
      if (!callerPresenceRow) return { ok: false, reason: BS.activePresenceRequired, serverNow };
    ...
    igniteTnt: mutation(async (ctx, requestJson: string) => {
      ...
      if (activeFuses.length >= TNT_MAX_ACTIVE_FUSES && activeAtCoordinate.length === 0) {
        return { ok: false, reason: BS.fuseCapacity, serverNow };
      }
      const blockInstanceToken = furnaceBlockInstanceToken(worldRows[0]);
    ...
    claimTntExplosion: mutation(async (ctx, requestJson: string) => {
      ...
      if (activeRows.length > TNT_MAX_ACTIVE_FUSES) return { ok: false, reason: BS.fuseCapacity, serverNow };
      ...
      for (const row of activeRows) {
        ...
        if (!active) return { ok: false, reason: BS.invalidFuse, serverNow };
    ...
    claimCreeperExplosion: mutation(async (ctx, requestJson: string) => {
      ...
      if (!pose || !motionMob || pose.kind !== "creeper") return { ok: false, reason: BS.unknownMob, serverNow };
      const authority: CreeperExplosionAuthority = {
        mobId: pose.mobId,
    ...
    rangedCombat: mutation(async (ctx, requestJson: string) => {
      ...
      if (charge.active && chargeRow && (
        ...
        return { ok: false, reason: BS.conflict, serverNow };
      }
