...
function textureSource(image, names, columns, rows, tileSize, inputName) {
  ...
  const packed = [];
  for (let index = 0; index < pixelIndexes.length;) {
    const control = packed.length;
    packed.push(0);
    let flags = 0;
    for (let bit = 0; bit < 8 && index < pixelIndexes.length; bit += 1) {
      let length = 0;
      let distance = 0;
      for (let source = Math.max(0, index - 4_095); source < index; source += 1) {
        let candidate = 0;
        while (candidate < 18 && index + candidate < pixelIndexes.length
          && pixelIndexes[source + candidate] === pixelIndexes[index + candidate]) candidate += 1;
        if (candidate > length) {
          length = candidate;
      ...
      if (length >= 3) {
        ...
        index += length;
      } else packed.push(pixelIndexes[index++]);
    }
    packed[control] = flags;
  }
  const paletteBase64 = Buffer.from(palette).toString("base64");
  const indexBase64 = Buffer.from(packed).toString("base64");
  return `// Generated by scripts/pixelate-texture-sheet.mjs from ${JSON.stringify(inputName)}.\n`
    ...
    + `const TEXTURE_ATLAS_INDEXES = (() => { const packed = atob(${JSON.stringify(indexBase64)}), indexes = new Uint8Array(${pixelIndexes.length}); let target = 0;\n`
    + `for (let source = 0; source < packed.length;) {\n`
    + `  const flags = packed.charCodeAt(source++);\n`
    + `  for (let bit = 0; bit < 8 && source < packed.length; bit += 1) {\n`
    + `    if (flags & 1 << bit) { const value = packed.charCodeAt(source++) * 256 + packed.charCodeAt(source++), length = (value >> 12) + 3, distance = value & 4_095; for (let copy = 0; copy < length; copy += 1) indexes[target] = indexes[target++ - distance]; }\n`
    + `    else indexes[target++] = packed.charCodeAt(source++);\n`
    + `  }\n`
    + `}\n`
    + `return indexes; })();\n`
    ...
    + `  const source = TEXTURE_ATLAS_INDEXES[pixel] * 4;\n`
