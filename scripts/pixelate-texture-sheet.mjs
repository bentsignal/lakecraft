...
function textureSource(image, names, columns, rows, tileSize, inputName) {
  ...
  const paletteBase64 = packStaticBytes(palette);
  const indexBase64 = packStaticBytes(pixelIndexes);
  return `// Generated by scripts/pixelate-texture-sheet.mjs from ${JSON.stringify(inputName)}.\n`
    ...
    + `const TEXTURE_ATLAS_PALETTE = decodeStaticBytes(${JSON.stringify(paletteBase64)}, ${palette.length});\n`
    + `const TEXTURE_ATLAS_INDEXES = decodeStaticBytes(${JSON.stringify(indexBase64)}, ${pixelIndexes.length});\n`
    ...
    + `  for (let channel = 0; channel < 4; channel += 1) TEXTURE_ATLAS_RGBA[target + channel] = TEXTURE_ATLAS_PALETTE[source + channel];\n`
    + `}\n`;
}
