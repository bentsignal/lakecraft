...
assert.match(sprites, /health:[\s\S]*hunger:[\s\S]*armor:/, "all three survival meter families ship exact sprites");
assert.equal((sprites.match(/data:image\/png;base64/g) ?? []).length, 1, "one compact shared data prefix backs all sprites");
assert.equal((sprites.match(/empty: PNG/g) ?? []).length, 3);
assert.equal((sprites.match(/full: PNG/g) ?? []).length, 3);
assert.equal((sprites.match(/half: PNG/g) ?? []).length, 3);
