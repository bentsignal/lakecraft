...
assert.equal(parseLocalCommand("/gamemode creative", { changeGameMode: false, giveItems: true }).ok, false);
assert.equal(parseLocalCommand("/give dirt", { changeGameMode: true, giveItems: false }).ok, false);
...
assert.ok(app.includes('event.code === "KeyT" || event.code === "Enter"'));
...
assert.equal((engine.match(/options\.canTakePlayerDamage\?\.\(\) !== false/g) ?? []).length, 4,
  "mob, creeper, fall, and TNT damage all honor creative invulnerability before mutating health");
