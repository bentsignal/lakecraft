...
assert.equal(engineSource.match(/gl\.drawArrays\(/g)?.length, 17,
  "world slabs stay batched; only bounded emissive, fluid, and destroy-stage passes draw independently");
...
assert.match(singlePlayerSaveSource, /candidate\.block, BLOCK\.AIR, BLOCK\.NETHER_WART_BLOCK/,
