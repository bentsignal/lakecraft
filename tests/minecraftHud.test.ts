...
assert.equal(crosshairCss.match(/clip-path: polygon/g)?.length, 1,
  "the reticle is one authored pixel silhouette without a synthetic outline");
assert.ok(!crosshairCss.includes("background: #111") && crosshairCss.includes("background: #fff"),
  "reticle uses the requested unoutlined white Minecraft silhouette");
assert.ok(!crosshairCss.includes("6px 0,10px 0") && crosshairCss.includes("7px 1px,9px 1px"),
  "the single plus stays aligned to integer pixel coordinates");
