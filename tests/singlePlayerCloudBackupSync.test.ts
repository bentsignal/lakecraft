...
for (const wire of [[1, 10, [], []], [2, 10], [3, 10]]) assert.deepEqual(parseSinglePlayerCloudQueryWire(wire), wire);
for (const wire of [[1, 10, []], [1, 10, [], [], 0], [4, 10], [2, "10"], [1, 8_640_000_000_000_001, [], []],
  [1, 10, new Array(1), []], {}, null]) assert.equal(parseSinglePlayerCloudQueryWire(wire), null);
const mutationWires = [[1, "1", 10], [2, 10], [3, "cloud_capacity", 10], [3, "world_limit", 10],
  [4, 10], [5, "conflict", 10], [6, 1, 10]];
...
for (const wire of [[1, "1"], [1, "1", 10, 0], [7, "conflict", 10], [1, "0", 10],
