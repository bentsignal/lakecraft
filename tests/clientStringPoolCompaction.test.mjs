...
assert.equal(
  ...
  COMPACT_CLIENT_REPEATED_SOUND_RUNTIME_DELTA.previousOccurrences,
...
assert.equal(
  ...
  COMPACT_CLIENT_REPEATED_SOUND_RUNTIME_DELTA.previousUniqueValues,
  "runtime atlas icon reconstruction accounts for the promoted top value",
);
assert.equal(
  COMPACT_CLIENT_REPEATED_SOUND_RUNTIME_DELTA.previousOccurrences
    + COMPACT_CLIENT_REPEATED_SOUND_RUNTIME_DELTA.occurrenceDelta,
  COMPACT_CLIENT_REPEATED_STRING_OCCURRENCES,
  "sampled audio accounts for its exact repeated occurrence increase",
);
assert.equal(
  COMPACT_CLIENT_REPEATED_SOUND_RUNTIME_DELTA.previousUniqueValues
    + COMPACT_CLIENT_REPEATED_SOUND_RUNTIME_DELTA.uniqueValueDelta,
  COMPACT_CLIENT_REPEATED_STRING_UNIQUE_VALUES,
  "sampled audio accounts for its exact repeated vocabulary increase",
);
assert.equal(
  COMPACT_CLIENT_LOW_FREQUENCY_SOUND_RUNTIME_DELTA.previousOccurrences
    + COMPACT_CLIENT_LOW_FREQUENCY_SOUND_RUNTIME_DELTA.occurrenceDelta,
  COMPACT_CLIENT_LOW_FREQUENCY_STRING_OCCURRENCES,
  "sampled audio accounts for its exact low-frequency occurrence increase",
);
assert.equal(
  COMPACT_CLIENT_LOW_FREQUENCY_SOUND_RUNTIME_DELTA.previousUniqueValues
    + COMPACT_CLIENT_LOW_FREQUENCY_SOUND_RUNTIME_DELTA.uniqueValueDelta,
  COMPACT_CLIENT_LOW_FREQUENCY_STRING_UNIQUE_VALUES,
  "sampled audio accounts for its exact low-frequency vocabulary increase",
);
assert.deepEqual(
  COMPACT_CLIENT_REPEATED_SKIN_STORAGE_CODEC_DELTA,
  {
