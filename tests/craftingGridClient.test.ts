...
assert.ok(drawer.includes("takeWorkspaceCraftingResult") && drawer.includes("stowInventoryWorkspace"), "crafting consumes exact grid slots and publishes a capacity-checked snapshot");
assert.ok(drawer.includes("onWorkspaceChange(stowed.snapshot") && drawer.includes("closeAndStow"), "grid and cursor contents are losslessly projected on every transition and close");
