...
assert.equal(WORLD_CHUNK_CODEC_MAX_BLOCK_TYPES, 1023);
...
assert.equal(sparseJson.v, 7);
assert.deepEqual(sparseJson.sections.map((section) => section.y), [0, 24]);
assert.ok(sparseFar.snapshotJson.length < 1_800, `two sparse v7 sections were ${sparseFar.snapshotJson.length} bytes`);
...
if (appended.ok) {
  assert.equal(JSON.parse(appended.snapshotJson).v, 7);
...
assert.ok(denseEncodeMs < 100, `dense v7 encode took ${denseEncodeMs.toFixed(2)}ms`);
assert.ok(denseDecodeMs < 100, `dense v7 decode took ${denseDecodeMs.toFixed(2)}ms`);
...
console.log(JSON.stringify({
  benchmark: "v7 ten-bit vertical-section world persistence",
...
console.log("lakecraft world chunk v7 tests: ok");
