...
const bed = createBedStructure({ x: 0, y: 44, z: 0 }, "east");
const baseEdits: WorldEdit[] = [
  { x: 0, y: 43, z: 0, block: BLOCK.STONE },
  { x: 1, y: 43, z: 0, block: BLOCK.STONE },
  { ...bed.foot, block: BLOCK.BED },
  { ...bed.head, block: BLOCK.BED },
  { x: 0, y: 45, z: 0, block: BLOCK.SAND },
  { x: 1, y: 45, z: 0, block: BLOCK.GRAVEL },
];
...
for (const selected of [bed.foot, bed.head]) {
  ...
  const live = createLiveEngine({
    ...
    twoBlockBeds: true,
    initialPose: { x: selected.x + 0.5, y: 44.02, z: 3.5, yaw: 0, pitch: -0.43 },
...
const rejected = createLiveEngine({
  ...
  initialBedStructures: [bed],
  twoBlockBeds: true,
  initialPose: { x: 0.5, y: 44.02, z: 3.5, yaw: 0, pitch: -0.43 },
  preserveInitialPose: true,
  getMiningDuration: () => 0,
...
const farBed = createBedStructure({ x: 240, y: 44, z: 240 }, "north");
