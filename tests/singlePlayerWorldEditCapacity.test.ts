...
for (let sample = 0; sample < 50_000; sample += 1) {
  assert.equal(tryCommitLocalWorldEdits(index, [{ x: 0, y: 1, z: 0, block: sample % 2 ? BLOCK.STONE : BLOCK.DIRT }], limit), true);
}
