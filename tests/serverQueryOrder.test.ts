...
assert.equal((serverSource.match(/\bnewestByIndex\(/g) ?? []).length, 158, "reviewed descending helper live set changed");
...
assert.equal((serverSource.match(/\.take\(2\)/g) ?? []).length, 93, "take(2) terminals are untouched");
assert.equal((serverSource.match(/\.take\(/g) ?? []).length, 134, "non-receipt take terminals are untouched");
