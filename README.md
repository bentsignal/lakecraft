...
World mining loot is coordinate-derived and replay-safe. Gravel resolves flint only for shovels; oak leaves resolve to the block only for 238-use iron shears, otherwise to a sparse one-in-twenty apple or nothing. Single-player and Lakebed world operations call the same pure resolver, while successful shears wear commits atomically with the block edit and rejected or replayed operations consume no durability.
