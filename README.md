...
- `server/index.ts` — Lakebed schema, auth-backed profiles, public external-server registrations, and short-lived scoped join tickets
- `apps/game-server/` — self-hostable Railway authority for movement, terrain edits, inventory, drops, chat, PvP, mobs, world settings, reconnect credentials, roles, and the per-world `/admin` console
...
Project intent and non-obvious engineering knowledge live in focused repository documentation and do not require UAV. Start with the [documentation index](./docs/README.md). The small [project backlog](./docs/backlog.md) is the only local record of open work; Git and merged pull requests preserve completed history.
...
Lakebed owns accounts, unique usernames, the public server directory, server ownership/registration, and 45-second scoped join tickets. After redeeming a ticket, the selected Railway world is the sole multiplayer gameplay authority: its portable SQLite database owns movement, chunk edits, inventory, drops, chat, health/PvP, mobs, world time/settings, reconnect credentials, and per-world Survival/Creative roles. Those records are not polled from or mirrored into Lakebed, so ordinary multiplayer play consumes no Lakebed query or mutation quota. The exact operational split and self-hosting flow are documented in [docs/railway-multiplayer-server.md](./docs/railway-multiplayer-server.md).
...
World mining loot is coordinate-derived and replay-safe. Gravel resolves flint only for shovels; oak leaves resolve to the block only for 238-use iron shears, otherwise to a one-in-200 apple, a one-in-20 sapling, or nothing. Single-player and Railway-authoritative world operations call the same pure resolver, while successful shears wear commits atomically with the block edit and rejected or replayed operations consume no durability. Logs can be smelted into charcoal, and a shared fuel predicate keeps the furnace UI and authoritative transfer path aligned on coal and charcoal. Both fuels reuse the same trusted-time 80-second burn materializer and exact-replay receipts.

Single-player runs the shared bounded mob simulation locally. Survival multiplayer runs the corresponding ecology on Railway: at most eight passive animals in retained herds and four hostile mobs, with cave darkness active during the day and mixed cave/surface spawning at night. Railway checkpoints motion and health to SQLite, validates contact and player attacks, makes death/drop receipts idempotent, and streams one bounded snapshot to each browser; clients only render and interpolate that authority. Creative servers disable the population by default.

Sheep can be clipped by right-clicking with shears. In multiplayer the interaction is a discrete Railway-authoritative operation that reuses active presence/reach checks and a bounded receipt window; one accepted transaction changes the shared coat state, spends exactly one durability, and inserts a deterministic one-to-three wool into the same authoritative inventory. Replays return the current inventory without paying again, and ordinary mob snapshots carry the narrower pink sheared model without another network loop. Single-player uses the same visible state transition and inventory-capacity fence locally.
