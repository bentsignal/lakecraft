...
export function ChestDrawer({ open, chestInventory, playerInventory, busy = false, status, error, onClose, onTransfer }: ChestDrawerProps) {
  if (!open) return null;
  return (
    <div className="lc-chest-layer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      ...
      <section className="lc-chest" role="dialog" aria-modal="true" aria-labelledby="lc-chest-title">
        <header><div><small>SHARED LAKEBED CONTAINER</small><h2 id="lc-chest-title">Chest</h2></div><button onClick={onClose} type="button">Close · E</button></header>
        ...
        <p className={`lc-chest-status${error ? " is-error" : ""}`} role={error ? "alert" : "status"}>{error || status || "Every transfer is saved to the shared chest through Lakebed."}</p>
      </section>
    </div>
  );
}
