      ...
      {Array.from({ length: inventory.length }, (_, visualIndex) => {
        ...
        return (
          <button
            ...
            className={`lc-chest-slot${stack && busy ? " is-disabled" : ""}`}
            disabled={!stack}
            key={index}
            onClick={() => { if (!busy) onTransfer(direction, index); }}
            type="button"
          >
