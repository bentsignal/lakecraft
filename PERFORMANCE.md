...
Chickens likewise add no table, query, mutation, checkpoint cadence, allocation, or draw call. The canonical checkpoint now contains twelve passive and four hostile slots, cycling evenly across four kinds in each group. A chicken uses nine boxes inside the existing 12-box maximum: body, head, tail, beak, wattle, two wings, and two legs. Its feather loot reuses the exact-once mob death receipt path, and arrow crafting remains an ordinary inventory workspace commit rather than a separate server operation. Retained pre-chicken checkpoints reseed once through the existing leased checkpoint mutation.
