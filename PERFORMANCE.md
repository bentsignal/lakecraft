...
Shears and apples also reuse the existing atomic world-operation path. Leaf loot is a pure hash of the canonical coordinate and held item: shears preserve the leaf, while other items produce an apple at a fixed one-in-twenty rate or no drop. The authoritative transaction applies one shears durability only after every block, revision, reach, inventory-capacity, and receipt fence passes. This adds no row, query, mutation type, timer, random source, or polling cadence.
