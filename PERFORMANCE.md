...
The capsule also reached a Lakebed packaging boundary: a direct deploy request exceeds 2 MiB because the current CLI snapshots repository metadata and emits inline source maps. `scripts/prepare-lakebed-deploy.mjs` uses Lakebed's bundled compiler to flatten the client and server into two minified entrypoints before the normal `npx lakebed deploy`. The textured release payload is 1,995,224 bytes, leaving roughly 100 KiB below the request ceiling; treat 2.0 MiB as a hard release gate until the upstream packager changes.
