...
Mob movement now uses one fixed-point 10 Hz shared reducer and a singleton Lakebed checkpoint row. The authority lease lasts 30 seconds and the holder advances the row every second: 60 checkpoint mutations/minute, one retained row, and no frame-loop writes or append growth. Each checkpoint also stores the canonical player/night input for the next interval, so every reader replays immutable inputs instead of rewriting the preceding timeline from its own request-time presence snapshot. Each in-world client intentionally re-subscribes to the authority query at 5 Hz (300 reads/minute) because Lakebed does not invalidate a time-derived query while every subscribed row is idle. Followers interpolate those snapshots in retained renderer arrays. This is request-quota-expensive by design: two clients can spend roughly 600 of the hosted 10,000 daily requests each minute before other actions. The 6,000-tick/10-minute replay has the golden hash `1d5bfbaae8e310f6`; duplicate hostile-damage claims reuse bounded combat receipts and apply once.
...
- Persist one mob checkpoint and its immutable replay input every second under the session-bound lease; never write mob state from the render loop.
